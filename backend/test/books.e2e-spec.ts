...
describe('BooksController (e2e)', () => {
  ...
  it('/api/books/genres (GET)', async () => {
    const response = await request(app.getHttpServer()).get('/api/books/genres').expect(200);
  ...
  it('/api/books/:id (GET) - Not Found', async () => {
    await request(app.getHttpServer()).get('/api/books/non-existent-id').expect(404);
  });
});
