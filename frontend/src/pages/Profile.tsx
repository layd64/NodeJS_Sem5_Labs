  ...
  if (!isAuthenticated) {
    return (
      <section>
        <h2>Профіль</h2>
        <p>
          Будь ласка, <Link to="/login">увійдіть</Link>, щоб переглянути профіль.
        </p>
      </section>
    );
  }
    ...
    <>
      <section id="profile-section">
        ...
        {profile && (
          <div>
            <p>
              <strong>Ім'я:</strong> {profile.name}
            </p>
            <p>
          ...
          <div className="books-grid">
            {savedBooks.map((book) => (
              <div key={book.id} className="book-card">
                ...
                <button className="btn-danger" onClick={() => handleRemoveSavedBook(book.id)}>
                  Видалити
                </button>
              </div>
            ))}
          ...
          <div id="reviews-container">
            {reviews.map((review) => (
              <div key={review.id} className="review-item">
                ...
                <p>{review.comment}</p>
                <div className="review-meta">{new Date(review.createdAt).toLocaleDateString()}</div>
              </div>
            ))}
          </div>
        )}
